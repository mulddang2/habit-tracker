...
원래는 "도그푸딩(KPI 측정)" 용도로 시작했지만, 첫 주에 동기화 관련 버그가 연쇄적으로 드러나면서 KPI 측정 단계까지 진입하지 못했다. 이 일지는 그 **안정화 사이클**을 있는 그대로 남긴 기록이며, 사용자 행동 KPI 측정은 별도 단계로 분리해 진행할 예정이다.
...
- 산출물 요약
  ...
  - 모든 수정에 회귀 테스트 동반 — 마감 시점 227 케이스 전부 통과
- 패턴화된 결함 유형
  - **mirror가 아닌 upsert** — hydrate의 `bulkPut`만으로는 서버 측 삭제가 전파되지 않음 (B1)
  - **캐시 무효화 누락** — hydrate(B2) 및 mutation `onSettled`(B4, `useDeleteHabit`도 동일 유형)에서 일부 React Query 키 무효화를 빠뜨림
  - **flush ↔ hydrate race** — fire-and-forget 방식의 flush와 hydrate가 경합해 삭제된 row가 되살아남 (B3a / B3b, locked ids 패턴으로 해결)
  - **사용자 경계 누수** — `signOut`이 supabase 세션만 끊고 로컬 IndexedDB·zustand persist를 남겨두면, 큐가 다음 계정으로 푸시되는 문제 (B5)
  - **head-of-line blocking** — 영구 실패한 항목(poison-pill)이 재시도 한도 없이 큐의 헤드를 점유 (B6, `MAX_SYNC_RETRIES` 도입)
...
- 프로세스 메모
  - 5/8 무렵 일지가 stash에 묻혀 마감 직전에야 발견됨 — 주간 일괄 커밋 정책상 커밋은 미루더라도 **작성한 즉시 워킹 트리에 두기(stash 금지)** 규칙을 다음 사이클부터 적용한다.
...
- 체크 항목에 기한(만료일)을 둘 수 있는 옵션 — 3일차에 떠오름. 단기 목표성 습관에 대응하기 위한 기능.
