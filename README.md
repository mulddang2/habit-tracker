...
- **캐시 무효화 단일화 (ADR-002)** — 서버 상태 책임을 TanStack Query 한 곳으로 모아, `byDate`/`byMonth`로는 잡히지 않던 주간 파생 키 무효화 누락을 단일 지점에서 해결했습니다 ([useHabitLogs.ts](src/hooks/useHabitLogs.ts)).
